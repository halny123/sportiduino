...
// 
// Функции Beep переделанны
// Сначало включается светодион на заданный период , измеряется напряжение после стабилизатора
// если оно <3.1В звуковой сигнал выдаваться не будет
// На основе прошивки 1.4.2 g
// ---------------   NRF24l01 ---------------
#include <avr/interrupt.h>   

bool rf24_online = false;    // Состояние модуля NRF24l01 (нет питания)
bool NO_BeepBuzer = false;   // При установке в запрещает работу звукового сигнала для предотвращения разряда батареи.
...
const uint16_t Vcc_no_Buz = 3100;  // Напряжение после стабилизатора ниже которого отключается дублирование звуком 
 ...
 * Идет ввключение светодиода и проверяется напряжение батареи если напряжение >3.1V 
 * подается звуковой сигнал иначе выставляем сигнал NO_BeepBuzer и не пищим
 *
 */
...
void beep_mark() {
  ...
  delay(200);
  digitalWrite (LED, HIGH);
  if (!NO_BeepBuzer) { tone (BUZ, 4000, 200);}
  digitalWrite (LED, LOW);
 ...
 * Если напряжение меньше 3.1 В, Устанавливаем флаг NO_BeepBuzer - для отключения бузера
 *
 */
...
void Check_Vcc() {
  ...
  if (value < Vcc_no_Buz) {   NO_BeepBuzer = true;   }
  else { NO_BeepBuzer = false; }
