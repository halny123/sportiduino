...
// resetFunc(); - Перегрузка только по прерыванию от WDT & Gerkon. Раньше была после каждого мастер чипа и по возврату из сна
// Поменял усиление антены на 38дБ по умолчанию
// Добавил измерение напряжения питания во время инициализации с выводом (без звука) на светодиод. 2 пачки миганий с паузой еденицы и десятые доли вольта. По окончанию длинный сигнал
// Измерение питания проходят при включенном звуковом и световом сигнале 1,5с
//
//  БАГИ 
//  void timeChip() - не проверяет корректность записи времени в часы
//
//
//
//
...
inline void beepStart()                  { beep(50, 2); delay(200); beep(50, 1); delay(300); beep(50, 3);delay(700); }  //-------- Сигнализируем о перегрузке станции
inline void beepEndSetup()               { beep(1000, 1 );}      // Завершение инициализации void setup()

inline void beepOk()                     { beep(200, 1); }       // Все хорошо
inline void beepWriteTimeToChipOk()      { beepOk(); }           // запись отметки в чип участника успешна.
inline void beepSleepOFF()               { beepOk(); }           // Станция вышла из спящего режима
inline void beepClearChipOk()            { beepOk(); }           // Очистка чипа успешна
inline void beepCheckChipOk()            { beepOk(); }           // Тестирование чипа (чип чист и инициализирован)

inline void beepDubleWriteTimeToChip()   { beep(300, 1); }       // Повторная попытка отметки того же КП в чип участника. (Номер последней станции совпадает с текущей)

// Оригинальные сигналы
// inline void beepPassChipOk()             { beep(500, 2); }       // обработки мастер-чипа смены пароля и байт настроек.
// inline void beepTimeChipOk()             { beep(500, 3); }       // обработки мастер-чипа времени
// inline void beepSleepChipOk()            { beep(500, 4); }       // обработки мастер-чипа перехода в сон
// inline void beepStantionChipOk()         { beep(500, 5); }       // обработки мастер-чипа записи номера(назначения) базовой станции ОК
// inline void beepDumpChipOk()             { beep(500, 6); }       // обработки мастер-чипа создания дампа 

// inline void beepEEPROMReadERROR()        { beep(50, 2); }        // ОШИБКА Считывание ячейки из внутренней памяти МК с учетом мажоритарного резервирования
// inline void beepSetTimeERROR()           { beep(50, 3); }        // ОШИБКА установки времени
// inline void beepPassMasterCardERROR()    { beep(50, 4); }        // ОШИБКА пароля на мастер чипе
// inline void beepMeasureVoltageERROR()    { beep(50, 5); }        // ОШИБКА - Низкое напряжение на батарее
// inline void beepStantionChipERROR()      { beep(50, 6); }        // ОШИБКА обработки мастер-чипа записи номера(назначения) базовой станции 
// inline void beepCheckChipERROR()         { beep(200, 3); }       // ОШИБКА тестирование чипа (чип не чист/не инициализирован)

// Альтернатива - ОШИБКИ - длинные\ Сообщения - короткие
inline void beepPassChipOk()             { beep(50, 2); }       // обработки мастер-чипа смены пароля и байт настроек.
inline void beepTimeChipOk()             { beep(50, 3); }       // обработки мастер-чипа времени
inline void beepSleepChipOk()            { beep(50, 4); }       // обработки мастер-чипа перехода в сон
inline void beepStantionChipOk()         { beep(50, 5); }       // обработки мастер-чипа записи номера(назначения) базовой станции ОК
inline void beepDumpChipOk()             { beep(50, 6); }       // обработки мастер-чипа создания дампа 
 

inline void beepEEPROMReadERROR()        { beep(500, 2); }        // ОШИБКА Считывание ячейки из внутренней памяти МК с учетом мажоритарного резервирования
inline void beepSetTimeERROR()           { beep(500, 3); }        // ОШИБКА установки времени
inline void beepPassMasterCardERROR()    { beep(500, 4); }        // ОШИБКА пароля на мастер чипе
inline void beepMeasureVoltageERROR()    { beep(500, 5); }        // ОШИБКА Низкое напряжение на батарее
inline void beepStantionChipERROR()      { beep(500, 6); }        // ОШИБКА обработки мастер-чипа записи номера(назначения) базовой станции 
inline void beepCheckChipERROR()         { }                      // ОШИБКА тестирование чипа (чип не чист/не инициализирован)
...
#define BUZZER_FREQUENCY 4000 // Частота работы Buzzerа, 0 - Buzzer со встроенным генератором

// ----------- Схема измерения VccBAT  если не используем (на плате нет) измерение питания на батарее ОБЯЗАТЕЛЬНО комменитуем --------------
#define ADC_IN         A0
#define ADC_ENABLE     A1
// ----------- Vcc end------------------

uint32_t  measureBatteryVoltage(bool beepEnabled = true);       // true - измерение питания батареи проводим с включенным бузером. false - только свет
bool      checkBattery(bool beepEnabled = true);                    // true - выводим на светодиод результат измерение питания батареи, иначе молчим

//  ----- Усиление антены  -->  0x00 - 18 dB,  0x01 - 23 dB, 0x02 - 18dB, 0x03 - 23dB, 0x04 - 33dB, 0x05 - 38dB, 0x06 - 43dB, 0x07 - 48dB
uint8_t gain = 0x05 << 4;      // Усиление антены RFID считывателя по умолчанию - 38dB
...
boolean deepsleep = false;           //On / off stantion. In off regime sleep 8 s вкл - режим сна.
...
// ---------------   NRF24l01 ---------------
// Serial.begin(9600);
beepStart(); // - Сигнализируем о перегрузке станции
//Serial.println( "Перегрузились " ); 

pinMode(VCC_NRF, INPUT);

// -------- инициализируем порты на вход если есть модуль измерения Vcc
...
// --------------- END  NRF24l01 ---------------
  ...
  delay(500); //is necessary to to reflash station. in sleep mode it is not possible.


 // Измеряем питание на батарее если апределены переменные
  #if (defined(ADC_IN) && defined(ADC_ENABLE))
    checkBattery();
  #endif
 

  beepEndSetup(); //The signal at system startup or reboote
}//end of setup
...
#if defined(ADC_IN) && defined(ADC_ENABLE) // Оставляем для исключение тока утечки черех цепь измерения напряжения питания
    pinMode(ADC_IN, INPUT);
    pinMode(ADC_ENABLE, INPUT);
#endif
// --------------- END  NRF24l01 ---------------
...
/*
 * Измерение напряжения. Включает диод на 3 секунд. Затем происходит измерение.
...
//  -------------------- Не знаю нужно ли измерять после выхода из сна не по геркону ?
//  #if (defined(ADC_IN) && defined(ADC_ENABLE))
//    checkBattery();
//  #else
//     voltage();
...
// --------------- Vcc check ------------------
bool checkBattery(bool beepEnabled) {  // возвращаем true - если напряжение больше минимально заданного
    uint32_t voltage = measureBatteryVoltage();
    uint8_t Vcc1 = int (voltage/1000);
    uint8_t Vcc2 = int ((voltage-Vcc1*1000)/100);
    const uint32_t minVoltage = 3600;
    wdt_reset();
    delay(250);
     if(beepEnabled) {
       beepBatteryVcc(Vcc1,Vcc2);  // ----------   Vcc Bat ----------- 
       wdt_reset();
      }   
...
void beepBatteryVcc (uint8_t  a, uint8_t b)
  { 
    delay (750);
    wdt_reset();
    beep_nobuzer(LED, 200, a); // --------------beep(200, a); 
    wdt_reset();
    delay (950);
    wdt_reset();
    beep_nobuzer(LED, 200, b); // ------------ beep(200, b); 
    wdt_reset();
    delay (1500);
  } 

  //--------------- beep no buzer
  void beep_nobuzer(const uint8_t ledPin, uint16_t ms, uint8_t n) {
    for(uint8_t i = 0; i < n; i++) {
        wdt_reset();
...
#if defined(ADC_IN) && defined(ADC_ENABLE)
uint32_t measureBatteryVoltage(bool buz_on) {
    ...
    digitalWrite(LED, HIGH);
    if(buz_on) tone(BUZ, BUZZER_FREQUENCY);  //  --------  проверка с нагрукой бузером
    wdt_reset();
    delay(1500);
